...
Six data domains: listening (Last.fm + Apple Music), running (Strava, all activity types), watching (Plex + Letterboxd + Trakt + manual), collecting (Discogs + Trakt), reading (Instapaper), places (Foursquare/Swarm). Each domain has its own sync worker (cron-triggered), database tables, and route handlers.
