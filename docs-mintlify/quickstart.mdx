...
The full machine-readable schema for every endpoint is published as an OpenAPI 3.1 document at the API origin -- no auth required:

```bash
curl https://api.rewind.rest/openapi.json
```

`https://api.rewind.rest/openapi.json` (root) redirects to the canonical `https://api.rewind.rest/v1/openapi.json`. Use it to generate clients, drive agent tools, or import into Postman / Insomnia.
