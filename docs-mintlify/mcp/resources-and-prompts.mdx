...
| Schema         | `rewind://schema`               | `/v1/schema`                                              |
...
The `rewind://schema` resource is the annotated database schema — every queryable table with its columns, types, semantic notes, and join keys, plus global conventions. Pin it into context (or call the `get_schema` tool) before writing a `query_rewind` SELECT.
