...
export const SCHEMA_DOC: SchemaDoc = {
  notes: [
    ...
    "Images: the shared `images` table is the ONLY source of artwork -- it keys on (domain, entity_type, entity_id) and every image lives there. The public CDN URL is composed as https://cdn.dinakartumu.com/cdn-cgi/image/<transforms>/<r2_key>, and image_version is appended as a cache-busting query param (?v=<image_version>). Domain tables do NOT carry a usable artwork column: movies.poster_path / shows.poster_path are raw TMDB paths against a different host, not R2 keys, and are null for many rows. Always join `images`. dominant_color/accent_color are hex strings; thumbhash is a compact placeholder blob. ONE exception: there is no backdrop in R2 (one asset per entity, the poster), so movies.backdrop_path / shows.backdrop_path are hotlinked straight from TMDB -- compose `'https://image.tmdb.org/t/p/w1280' || m.backdrop_path`, present for roughly half the films. SELECT it alongside a movie detail row and query_rewind renders it across the top of the detail card -- see the detail-card note for the full recipe.",
    "Single-entity detail card in query_rewind: ONE row carrying a poster/cover CDN URL plus several other fields renders as a poster-forward card -- artwork large and uncropped, title beneath it, and a year column, a watched_at/read_at-style date, and a 0-10 rating promoted into the header (the rating drawn as stars out of ten). Every remaining non-empty column follows as a labeled field list; null columns are dropped. For the full treatment on a film, ALSO select the TMDB backdrop and the artwork's dominant colour: the backdrop renders sharp across the top of the card and dissolves into that colour, which then carries the rest of the card. Copy-paste: `SELECT 'https://cdn.dinakartumu.com/cdn-cgi/image/width=500,height=750,fit=cover,format=auto/' || i.r2_key || '?v=' || i.image_version AS poster, m.title, m.year, m.runtime, m.tmdb_rating, w.watched_at, w.user_rating, 'https://image.tmdb.org/t/p/w1280' || m.backdrop_path AS backdrop_path, i.dominant_color FROM watch_history w JOIN movies m ON w.movie_id = m.id LEFT JOIN images i ON i.domain = 'watching' AND i.entity_type = 'movies' AND i.entity_id = CAST(m.id AS TEXT) WHERE m.title = 'Midnight Cowboy' ORDER BY w.watched_at DESC LIMIT 1`. The backdrop column MUST keep a name containing `backdrop` (that is what marks it as decoration rather than content); the colour is recognised by value, so any hex column works. Both are optional and degrade cleanly: no backdrop -> a flat card washed in the colour; no colour -> the still fades out instead. The same shape works for albums and artists -- swap the images join and drop the backdrop.",
  ...
  tables: [
    ...
    // ─── Watching (Plex + Letterboxd + Trakt) ────────────────────────
    {
      ...
      columns: [
        ...
        c(
          'backdrop_path',
          'text',
          "TMDB path, nullable -- prefix 'https://image.tmdb.org/t/p/w1280' for a wide still; the only hotlinkable artwork column"
        ),
      ],
      joins: [
    ...
    },
    {
      ...
      columns: [
        ...
        c('backdrop_path', 'text', 'TMDB path, nullable -- see movies'),
      ],
