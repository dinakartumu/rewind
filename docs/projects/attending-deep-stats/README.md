...
| `/players/:id/stats?season=N` is **optional**, default = career across all attended games                                                                                                     | **Changed during Phase 0.** Sample-size audit found no hitter has > 50 PAs in a single season at attended games. Career slices (Cal Raleigh 130 PA / 32 games; Kirby 238 BF / 10 starts) are where meaningful aggregates live. Matches natural-language phrasing. Response carries `scope: "career"` when omitted, `scope: "season", season: N` when present. See [coverage-baseline.md](./coverage-baseline.md).  |
...
| **No `sample_size_warning` flag** — response always includes raw `pa` (hitter) / `bf` (pitcher) and `games`                                                                                   | **Changed during Phase 0.** A boolean threshold of `pa<50` would fire on 97% of (hitter, season) pairs and `bf<60` on 98.7% of pitchers — a warning that always fires is signal noise. Pivot: model gets the raw numbers and the tool description tells it "single-season slices typically have small samples (max 50 PAs across the dataset). Always cite the sample size." Phasing decision shifts to the model. |
...
1. **Sample sizes might make stats read as misleading-precise.** `.247` looks like a baseline; in reality a few games can swing it. Phase 0 confirmed the magnitude — single-season slices have max 50 PAs in the entire dataset. Mitigation pivoted from a boolean warning to "always include `pa`/`bf`/`games` in the response body and tell the model to cite them." Validate during Phase 4 checkpoint by checking what phrasings the model picks in real conversation.
