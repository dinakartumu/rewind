...
- **wakatime:** cursor = next date to fetch, walking backward; chunk = 14 days of `syncWakatimeDay`. The floor comes from account metadata: `getAllTimeSinceToday().startDate` (fetched once per invocation). Stop (return null cursor) once the cursor walks below that floor, or on `WakatimeHistoryLimitError`. Empty-chunk termination is a FALLBACK used only when no floor is available (start_date absent) — with a floor present, an empty chunk above it keeps walking, so a >=27-day vacation gap does not silently truncate the backfill.
- **rescuetime:** cursor = day to end the chunk at (exclusive), walking backward; chunk = 1 month of days. The only natural terminator is an empty month. ALL API errors are rethrown at any point in the walk (surface as a route 500): day syncs are idempotent upserts, so the operator retries the SAME cursor on a transient blip — swallowing a mid-walk error as "end of history" would silently truncate.
- **github:** cursor = JSON string `{"phase":"contributions","year":2026}`, validated with a zod discriminated union (malformed JSON / missing page / unknown phase → `GithubBackfillCursorError`, mapped to a route 400, not 500). Contributions walk years backward via `getContributionDays` down to the account-creation year (`getUserCreatedAt`, one GraphQL call per chunk) UNCONDITIONALLY — a gap (all-zero) year no longer ends the phase, so older history across gap years is not dropped; the phase ends only once the year reaches/passes the creation year. `itemsSynced` counts only days with count > 0. Then `{"phase":"prs","page":1}` → search pages until exhausted (GitHub Search caps at 1000 results — when `page * 100 >= min(totalCount, 1000)` the phase is done; log a `[INFO]` line when totalCount exceeded 1000 so truncation is visible); then `{"phase":"issues","page":1}` same; then null. One phase-step per invocation.
