...
export function registerQueryTools(
  ...
  server.registerTool(
    'query_rewind',
    {
      ...
      description:
        "Run a read-only SQL SELECT against the Rewind SQLite database. FIRST call get_schema (or read the rewind://schema resource) to see the tables and columns. Single SELECT (or WITH … SELECT) only; a LIMIT is auto-applied (200 default, 500 max). Great for any cross-domain or ad-hoc question the specialized tools do not cover — e.g. joining watches to check-ins, or ranking sources by article count. It cannot write, run DDL, or read secret tables (API keys, OAuth tokens); those are rejected server-side. Returns column names and row tuples in structuredContent plus a compact table preview. In MCP Apps hosts it ALSO renders an interactive view auto-selected from the result shape (or forced via `view`): a calendar heatmap when the result is a daily-date (YYYY-MM-DD) column plus one numeric column, a polar clock (radial histogram) when it is an hour-of-day (0-23) or weekday column plus a count, big-number stat cards when a single row has numeric columns, a ranked list, card grid, or metric-sized cover mosaic when a CDN image-URL column pairs with a name/label column and a metric, a stacked bar or a sankey flow diagram when the result is a category/source plus a series/target plus a numeric value (three columns; stacked when the first column is ordinal/temporal, sankey when both are free-form categoricals), a scatter plot when the result is exactly two numeric columns, a histogram when the result is a single numeric column of raw values, a chart or a share-of-whole treemap when the result is one category-or-period column plus one numeric column (treemap once there are eight or more distinct categories), a tile-less point/route map plotted from lat/lng or polyline columns (no tiles, no external requests) with a density-map tab (binned graduated markers over the same points) whenever there are lat/lng points and a route-gallery of small-multiple route shapes when there are four or more polyline routes (a gallery being the default for four-plus routes), a streak strip highlighting consecutive-day streaks offered alongside the calendar heatmap on a daily-date column, a poster-forward single-entity detail card when a single row carries a CDN image URL column plus several other fields (the artwork large and uncropped, the primary name/title beneath it, a year column, a `watched_at`/`read_at`-style date, and a 0-10 rating promoted into the header as a subtitle and a star row, and every remaining non-empty column as a labeled field list -- so select those columns when you want the card to lead with them; for a film also select the TMDB backdrop (`'https://image.tmdb.org/t/p/w1280' || m.backdrop_path AS backdrop_path`) and the poster's `i.dominant_color`, and the still renders across the top of the card dissolving into that colour -- see the detail-card note in get_schema for a copy-paste query), and a styled table otherwise. When every table a query reads belongs to one service -- or a projected `source` column names the same one on every row -- the card draws its marks in that service's colour and labels itself with it; a result spanning services stays neutral, so there is nothing to pass or opt into. There is also a curated year-in-review WRAPPED view you request explicitly with view:\"wrapped\" (never auto-selected): build a UNION-ALL across domains that returns highlight rows with columns `section, label, value, image` (image optional), grouped by `section` — e.g. SELECT 'Top Artists' AS section, name AS label, playcount AS value, <cdn_url> AS image FROM lastfm_artists ORDER BY value DESC LIMIT 3 UNION ALL SELECT 'Top Films', title, user_rating, <cdn_url> FROM watch_history … UNION ALL SELECT 'Miles Run', CAST(SUM(distance) AS TEXT)||' miles', SUM(distance), NULL FROM strava_activities WHERE started_at LIKE '2024%'; add a leading SELECT 'Year' AS section, '2024 in review' AS label, NULL, NULL to title the card. The wrapped view groups the rows by section and renders each as a mini panel (ranked list with covers when images are present, else a labeled stat); unknown sections render generically and missing images fall back to text. To include album art or posters in the answer, SELECT the composed CDN image URL — see the images-table note in get_schema; query_rewind renders any https://cdn.dinakartumu.com image URLs in the results as inline thumbnails (first 8 distinct, in row order). Set embed_art:true to ALSO get those matched CDN image URLs back as small base64 WebP data URIs in structuredContent.art (a map keyed by the original URL exactly as it appears in the row) — inline them when authoring a sandboxed artifact whose iframe cannot fetch the CDN directly; look up each row art URL in art[url]. It is downsampled (64px) and opt-in because it adds payload; leave it false for normal data queries.",
      inputSchema: {
        sql: z
          .string()
          .min(1)
